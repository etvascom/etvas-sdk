...
const validations = {
  ...
  debug: (value, context) => {
    ...
    if (value === null || Array.isArray(value) || typeof value !== 'object') {
      throw new Error(
        `${PREFIX} ${context} error: debug config should be an object`
      )
    }
    const { suppressSignatureCheck } = value

    if (suppressSignatureCheck === undefined) {
      return true
    }
    const sscType = typeof suppressSignatureCheck

    if (sscType === 'object' || sscType !== 'boolean') {
      throw new Error(
        `${PREFIX} ${context} error: suppresSignatureCheck config should be a boolean`
      )
    }
