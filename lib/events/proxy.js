...
const _getDebugConfig = () => {
  const { suppressSignatureCheck } = config.get('debug') || {}

  return {
    suppressSignatureCheck:
      !!process.env.ETVAS_SDK_DEBUG && !!suppressSignatureCheck
  }
}
...
const createProxyHandler = () => async (req, res) => {
  ...
  const { suppressSignatureCheck } = _getDebugConfig()
  ...
  if (!suppressSignatureCheck) {
    ...
    const canonical = JSON.stringify(req.rawBody || req.body)
  ...
  try {
    ...
    const jsonErrorResponse = { error: err.message }
    if (_isAxiosError(err)) {
      jsonErrorResponse.proxyHttpStatus = err.response.status
      jsonErrorResponse.proxyHttpData = err.response.data
    }
