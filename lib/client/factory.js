...
const _getTokenFromBearer = (bearer) =>
  bearer.indexOf('Bearer ') === 0 ? bearer.substr(7) : bearer
...
module.exports = (options) => {
  ...
  const putExternalData = async (key, value) => {
    ...
    const response = await xhr.put(`/organizations/external-data/${key}`, {
      data
    })
   ...
   */
  const getExternalPurchaseData = async (bearer) =>
    getExternalData(_getPurchaseIdentifier(bearer))
   ...
   */
  const putExternalPurchaseData = async (bearer, value) =>
    putExternalData(_getPurchaseIdentifier(bearer), value)
